...
_SCRAPER_RESOLUTION = flags.DEFINE_integer(
    "scraper_resolution",
    4,
    "How many hours a single dataset entry represents. " "See purpleair_scraper.py",
)
...
class TrainingSet:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            len(self.history) == expected_history
            and len(self.future) == expected_prediction
        )
...
def _write_csv(
    data: list[Measurement] | list[TrainingSet], outfile: pathlib.Path
) -> None:
...
def _get_time_bracketed_entries(
    start: datetime.datetime, end: datetime.datetime, data: list[Measurement]
):
    ...
    for measurement in data:
        t = TrainingSet(
            ...
            history=_get_time_bracketed_entries(
                measurement.get_history_start(), measurement.timestamp - epsilon, data
            ),
            future=_get_time_bracketed_entries(
                measurement.timestamp + epsilon, measurement.get_prediction_end(), data
            ),
        )
...
def main(argv: list[str]) -> None:
    if len(argv) != 2:
        raise app.UsageError(
            f"Invalid args; usage: {argv[0]} <input file> [optional flags]"
        )
    ...
    logging.info(
        "Created training data set with %d entries and validation data set with %d entries",
        len(training_data),
        len(validation_data),
    )
